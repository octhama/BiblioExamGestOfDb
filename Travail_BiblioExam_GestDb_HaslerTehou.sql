...
BEGIN
    IF NOT EXISTS (SELECT * FROM livre WHERE bookcode = pcd_bookcode) THEN
        ...
        INSERT INTO exemplaire(numex, prix, declasse, id_etat, bookcode)
        VALUES (pcd_numex, pcd_prix, pcd_declasse, pcd_id_etat, pcd_bookcode);
...
CALL ajout_exemplaire(3, '15.99'::money, false, 1, 'MP25014554'::varchar(10), '978-2-1234-5680-3'::varchar(13),
                      'Le Seigneur des Anneaux'::varchar(100));
CALL ajout_exemplaire(4, '50.99'::money, true, 2, 'IX25014664'::varchar(10), '978-6-1234-5680-1'::varchar(13),
                      'Dutch for Dummies'::varchar(100));
CALL ajout_exemplaire(5, '70.99'::money, false, 3, 'QQ25014554'::varchar(10), '978-9-1234-5680-6'::varchar(13),
                      'La Wallonie en 1000 questions'::varchar(100));
CALL ajout_exemplaire(6, '25.99'::money, false, 1, 'MX30004554'::varchar(10), '999-2-222-5555-3'::varchar(13),
                      'La Belgique en 1000 questions'::varchar(100));
...
CALL ajout_livre('QQ25014554'::varchar(10), '978-9-1234-5680-6'::varchar(13),
                 'La Wallonie en 1000 questions'::varchar(100));

CALL ajout_livre('XX00004554'::varchar(10), '333-2-4444-6666-0'::varchar(13),
                 'Bruxelles et ses mystères'::varchar(100));
...
BEGIN
    RETURN QUERY
        SELECT c.nom,
               c.prenom, -- nom et prénom du client
               (SELECT a.libelle
                FROM adresse a
        ...
        WHERE NOT EXISTS (SELECT *
                          FROM habite h2
                          WHERE h2.id_client = c.id_client -- même client
                            AND h2.datemodif > h.datemodif -- mais date plus récente
        );
END;
$$;
        ...
        LOOP
            x_prix_emprunt := ((x_date_retour - x_date_emprunt + 1) * 2 *
                               (SELECT ratio
                                FROM etat
                                WHERE id_etat =
                                      (SELECT etat_emprunt FROM emprunte WHERE emprunte.id_emprunte = x_id_emprunte))) +
                              CASE
                                  WHEN
...
-- 9. Créer un trigger 'before_suppression_client'

-- Création du trigger 'before_suppression_client'

CREATE OR REPLACE FUNCTION before_suppression_client()
    RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
BEGIN
    IF EXISTS (SELECT *
               FROM emprunte
               WHERE id_client = OLD.id_client) THEN
        RAISE EXCEPTION 'Le client a des emprunts en cours';
    ELSE
        RAISE NOTICE 'Le client n''a pas d''emprunts en cours';
    END IF;
    RETURN OLD;
END;
$$;

CREATE TRIGGER before_suppression_client
    BEFORE DELETE
    ON client
    FOR EACH ROW
EXECUTE FUNCTION before_suppression_client();

-- Utilisation du trigger 'before_suppression_client'
DELETE
FROM client
WHERE id_client = 1;

-- 10. Créer un trigger 'after_suppression_client'

-- Création du trigger 'after_suppression_client'

CREATE OR REPLACE FUNCTION after_suppression_client()
    RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
...
CREATE TRIGGER after_suppression_client
    AFTER DELETE
    ON client
    FOR EACH ROW
EXECUTE FUNCTION after_suppression_client();

-- Utilisation du trigger 'after_suppression_client'
DELETE
FROM client
WHERE id_client = 2;

create procedure routine_name(arg1 int)
as
$$
begin
    -- todo: implement
...
create function routine_name(arg1 int) returns varchar
as
$$
begin
    -- todo: implement
